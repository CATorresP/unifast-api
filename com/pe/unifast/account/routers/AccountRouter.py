...
async def get_account_by_id(accountId:int, db: Annotated[Session, Depends(get_db_session)],token: Annotated[str, Depends(oauth2_scheme)]):
...
@account_router.put("/account/{accountId}",response_model=AccountResponseDto)
async def update_account_by_id(accountId:int, accountDto: AccountDto, db: Annotated[Session, Depends(get_db_session)], token: Annotated[str, Depends(oauth2_scheme)]):
    verify=AuthService(db).TokenManager.decode(token)
    logger.info(verify)
    accountService = AccountService(db)
    account_response_dto = accountService.update_account(accountId, accountDto)
    return account_response_dto

@account_router.delete("/account/{accountId}")
async def delete_account_by_id(accountId:int, db: Annotated[Session, Depends(get_db_session)], token: Annotated[str, Depends(oauth2_scheme)]):
    verify=AuthService(db).TokenManager.decode(token)
    logger.info(verify)
    accountService = AccountService(db)
    accountService.delete_account(accountId)
    return

@account_router.post("/account",response_model=AccountResponseDto)
async def create_account(db: Annotated[Session, Depends(get_db_session)], accountDto: AccountDto):
